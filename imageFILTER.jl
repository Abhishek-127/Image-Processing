...
# filter_sharpCUSM()      - Cubic unsharp masking
#
# ---------------------------------
# Basic filters
# ---------------------------------
...
function filter_GAUSSIAN(img,sigma=1,width=3)
...
function filter_sharpUSMgauss(img,sigma=1,width=3)
...
function filter_sharpUMOSLap(img,width=3)
...
function filter_sharpUSMLofG(img, sigma=1, width=3)
...
# Function to perform Cubic USM
#  Description
#    A good value for lambda is 0.0015, as per suggested in the reference.
#    Non-separable mask and lambda=0.0015 default parameters
#
#  Ref(s):
...
#******* results look weird ********
function filter_cubicUSM(img, mode=1, lambda=0.0015)

    dx,dy = size(img)
    imgE = copy(img)
    imgK = zeros(Float16, dx, dy)
    println(size(imgK), size(img))

   # Separable cubic unsharp masking (NS-CUM) (Eq.7)
    if (mode == 1)
        for i=2:dx-1, j=2:dy-1
            imgK[i,j] = (img[i-1,j] - img[i+1,j])^2.0 *
                        (2.0*img[i,j] - img[i-1,j] - img[i+1,j]) +
                        (img[i,j-1] - img[i,j+1])^2.0 *
                        (2.0*img[i,j] - img[i,j-1] - img[i,j+1])
        end
    end
    ...
    if (mode == 2)
        for i=2:dx-1, j=2:dy-1
            imgK[i,j] = (img[i-1,j] + img[i+1,j] - img[i,j-1] - img[i,j+1])^2.0 *
                        (4 * img[i,j] - img[i-1,j] - img[i+1,j] - img[i,j-1] - img[i,j+1])
        end
    end

    for i=2:dx-1, j=2:dy-1
        newPixel = img[i,j] + imgK[i,j]*lambda
        if (newPixel < 0)
            imgE[i,j] = 0
        elseif (newPixel > 255)
            imgE[i,j] = 255
        else
            imgE[i,j] = round(UInt8, newPixel)
        end
    end
